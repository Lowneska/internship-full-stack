...
# if ai_mode is set to "mock" we use mock data
# otherwise we try to use OpenAI API if the key is set
AI_MODE = os.getenv("AI_MODE", "").lower()
OPENAI_KEY = os.getenv("OPENAI_API_KEY")


############################################
# Mock mode — Used when AI_MODE=mock or no API key
# since I don't have access to OpenAI API because I don't have a subscription
# I couldn't test the real OpenAI integration
############################################
...
############################################
# Real OpenAI mode — with a valid subscription key
############################################
...
if AI_MODE != "mock" and OPENAI_KEY:
    ...
    ############################################
    # for now we default to mock mode if no OpenAI key
    ############################################
    def generate_program_from_text(
        user_text: str, max_retries: int = 3
    ) -> ProgramResponse:
        print("AI_MODE=mock → Returning mock workout program")
        return mock_program()
